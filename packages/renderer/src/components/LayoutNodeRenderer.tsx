...
export function LayoutNodeRenderer({
  ...
  if (node.component === "Module" && node.props.ref && moduleMap) {
    ...
    return <div className="clapp-error">Module not found: {ref}</div>;
  }

  const Component = componentMap[node.component];
  if (!Component) {
    return (
      <div className="clapp-error">Unknown component: {node.component}</div>
    );
  }
  ...
  const children = node.children.map((child, i) => {
    ...
    return (
      <LayoutNodeRenderer key={i} node={child} moduleMap={moduleMap} />
    );
  });
